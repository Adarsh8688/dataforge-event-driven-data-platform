...
spark = (
    ...
    .appName("dataforge-customers-silver")
...
latest_record_window = Window.partitionBy("customer_id").orderBy(
...
customers_silver.write.mode("overwrite").parquet(str(SILVER_PATH))

print(f"Bronze event count: {bronze.count()}")
print(f"Silver customer count: {customers_silver.count()}")
