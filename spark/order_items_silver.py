...
spark = (
    ...
    .appName("dataforge-order-items-silver")
...
def decode_money_value(value):
    ...
    Debezium currently stores the NUMERIC value as:
        Base64 -> signed big-endian integer

    Olist monetary fields use scale 2, so:
...
# Normalize Debezium PostgreSQL NUMERIC values.
order_items_silver = (
    order_items_silver
    .withColumn(
        "price",
        decode_money(F.col("price")),
    )
    .withColumn(
        "freight_value",
        decode_money(F.col("freight_value")),
    )
)
...
print(f"Silver order item count: {order_items_silver.count()}")
...
print(f"Duplicate order item keys: {duplicate_count}")
